...
export const getThemeTokenVariants = (propertyPath: string) => {
  ...
  const val = keys.reduce<AnyObject>((obj, key) => {
    if (obj && typeof obj === 'object') {
      return (obj[key] as AnyObject) ?? {}
    }
    return {}
  }, themeTokens as AnyObject)
 ...
 */
export const addColorChannels = <T extends AnyObject>(
  obj: T
): AddChannelToLeaf<T> => {
  ...
  if (isLeafObject) {
    ...
    for (const [key, value] of Object.entries(obj)) {
      result[key] = value
      // Add channel property for both string values (actual colors) and null values (contract definition)
      if (typeof value === 'string') {
        result[`${key}Channel`] = color(value).rgb().array().join(' ')
      } else if (value === null) {
        result[`${key}Channel`] = null
      }
    }
  } else {
    ...
    for (const [key, value] of Object.entries(obj)) {
      if (typeof value === 'object' && value !== null) {
        result[key] = addColorChannels(value as AnyObject)
      } else {
        result[key] = value
      }
    }
