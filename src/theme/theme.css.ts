import { HtmlDataAttribute, ThemeColorPresets, ThemeMode } from '@/theme/type'
import {
  assignVars,
  createGlobalTheme,
  createThemeContract,
...
import { addColorChannels } from './shared'
...
for (const preset of Object.values(ThemeColorPresets)) {
  globalStyle(`:root[${HtmlDataAttribute.ColorPalette}=${preset}]`, {
    vars: assignVars(themeVars.colors.palette.primary, {
      ...addColorChannels(
        presetsColors[preset as unknown as keyof typeof presetsColors]
      ),
    }),
  })
}
